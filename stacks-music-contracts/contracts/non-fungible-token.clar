...
(define-map traits-map uint { topic: (string-ascii 10), title: (string-ascii 10), length: uint })
...
;; Mint a new NFT.
(define-public (mint (recipient principal) (topic (string-ascii 10)) (title (string-ascii 10)) (length uint))
  ...
  (let ((token-id (+ (var-get last-token-id) u1)))
    ...
    (try! (nft-mint? music-nft token-id recipient))

    (map-insert traits-map token-id {topic: topic, title: title, length: length})
